...
const AddFolderModal = ({ isOpen, onClose, albumName, onSuccess }) => {
  const [inputAlbum, setInputAlbum] = useState(albumName || "");
  const [year, setYear] = useState(new Date().getFullYear().toString());
  const [category, setCategory] = useState("");
  const [isSaving, setIsSaving] = useState(false);

  // Update inputAlbum if prop changes
  React.useEffect(() => {
    if (isOpen) {
      setInputAlbum(albumName || "");
    }
  }, [isOpen, albumName]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const finalAlbum = albumName || inputAlbum;
    if (!finalAlbum || !year || !category) return;
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white uppercase tracking-tight">
              {albumName ? "Add New Folder" : "Create New Album"}
            </h2>
          </div>
          <button
            onClick={onClose}

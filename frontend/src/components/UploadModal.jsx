...
const UploadModal = ({ isOpen, onClose, album, year, category, onSuccess }) => {
  ...
  // Auto-scroll to bottom when upload starts to show progress bar
  useEffect(() => {
    if (isUploading && scrollContainerRef.current) {
      setTimeout(() => {
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700">
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              {inputCategory
                ? `Upload to ${inputCategory}`
                : album || inputAlbum
                  ? `Upload to ${album || inputAlbum}`
                  : "New Upload"}
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              {inputCategory && (album || inputAlbum)
                ? `Album: ${album || inputAlbum}`
                : "Provide context to organize these files."}
            </p>
          </div>
          <button
            onClick={handleClose}
          ...
          >
            ...
            <div className="mb-6 relative">
              <label
                ...
                onDrop={handleDrop}
                className={cn(
                  "flex flex-col items-center justify-center w-full px-6 pt-5 pb-6 border-2 border-dashed rounded-xl transition-all cursor-pointer focus-within:ring-2 focus-within:ring-indigo-500",
                  isDragging
                    ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 scale-[1.02] shadow-lg shadow-indigo-500/10"
                    : "border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900/50 hover:bg-gray-100 dark:hover:bg-gray-800",
                )}
              >
                <UploadCloud
                  className={cn(
                    "mx-auto h-12 w-12 mb-2 transition-transform duration-300",
                    isDragging
                      ? "text-indigo-600 scale-110 animate-bounce"
                      : "text-indigo-400",
                  )}
                />
